...
export const useParseData = () => {
    ...
    const getTransImage = (item: any, languageCode: string, prop: string) => {
        if (
            ...
            item.translations.find(
                (ite: any) => ite.languages_code === languageCode
            )
        ) {
            const data = item.translations.find(
                (ite: any) => ite.languages_code === languageCode
            )
            if (data[prop]) {
                return data[prop]
    ...
    const getTransMultiImage = (
        ...
        if (
            ...
            if (data[prop] && data[prop][0]) {
                return data[prop].map((ite: any) => ite.directus_files_id)
            }
            else
                return null
        } else {
    ...
    const calTotalPageJSON = (result: any, limit: any) =>
        result && result.length > 0
            ? calTotalPage(result[0].count.id, limit)
            : 0
    const calTotalResultJSON = (result: any) =>
    ...
    const getFieldGallery = (item: any) => {
        if (item && Array.isArray(item) && item.length > 0 && item.every(ite => typeof ite === 'object' && !!ite.directus_files_id)) {
            return item.map(ite => ite.directus_files_id)
        } else return item
    }
    ...
    const checkItemArrayIsObject = (array: any) => {
        return array && Array.isArray(array) && array.length > 0 && array.every((item: any) => typeof item === 'object')
    }
